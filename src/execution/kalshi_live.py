...
class KalshiLiveEngine:
    ...
    async def run(self):
        if not self._private_key or not self.s.kalshi_api_key_id:
            ...
            return
        logger.info(
            "Kalshi LIVE execution ON — max $%.2f per order",
            self.s.kalshi_live_max_order_usd,
        )
        await asyncio.gather(self._run_opens(), self._run_closes())
    ...
    async def _place_order(self, sig: Signal):
        opp = sig.opportunity
        cost_usd = min(sig.position_size_usd, self.s.kalshi_live_max_order_usd)
        if cost_usd < 0.50:
        ...
        if entry <= 0 or entry >= 1:
            ...
            return
        count = max(1, int(cost_usd / entry))
        price_dollars = f"{min(0.99, entry + 0.01):.4f}"
    ...
    async def _place_close(self, payload: dict):
        ...
        if side_str not in ("yes", "no"):
            return
        price = max(0.01, min(0.99, float(sell_price) - 0.02))
